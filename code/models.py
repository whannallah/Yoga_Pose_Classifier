...
class YourModel(tf.keras.Model):
    ...
    def __init__(self):
        ...
        self.architecture = [
               # Layer 1, Convolution and MaxPooling
               Conv2D(filters=32, kernel_size=3, strides=1, activation="relu", padding="same", input_shape=(hp.img_size, hp.img_size, 1)), 
               Conv2D(filters=32, kernel_size=3, strides=1, activation="relu", padding="same"),
               MaxPool2D(2, name="layer1_pool"),
               Dropout(0.2),
               # Layer 2, Convolution and MaxPooling
               Conv2D(filters=64, kernel_size=3, strides=1, activation="relu", padding="same"),
               Conv2D(filters=64, kernel_size=3, strides=1, activation="relu", padding="same"),
               MaxPool2D(2, name="layer2_pool"),
               Dropout(0.2), 
               # Dropout layer
               Conv2D(filters=128, kernel_size=3, strides=1, activation="relu", padding="same"),
               Conv2D(filters=128, kernel_size=3, strides=1, activation="relu", padding="same"),
               Conv2D(filters=128, kernel_size=3, strides=1, activation="relu", padding="same"),
               MaxPool2D(2, name="layer3_pool"),
               Dropout(0.2), 
               
               Conv2D(filters=256, kernel_size=3, strides=1, activation="relu", padding="same"),
               Conv2D(filters=256, kernel_size=3, strides=1, activation="relu", padding="same"),
               Conv2D(filters=256, kernel_size=3, strides=1, activation="relu", padding="same"),
               MaxPool2D(2, name="layer4_pool"),
               Dropout(0.2), 
               # Flattening layer
               Flatten(), 
               # Densening layer
